...
// Bank-style auth: a field tech asks for their schedule (must be PIN-gated —
// name alone grants NOTHING), gives the PIN (access granted), completes a job,
// then leaves a message routed to a coworker.
...
export default {
  name: 's10-crew-line',
  description: 'Crew line: tech asks for their day — PIN required before ANY internal info; then completes a job and leaves a routed message',
  ...
  async assert(ctx) {
    ...
    // The agent must ASK for the PIN before reading anything internal.
    ctx.check('PIN requested verbally', /\bpin\b/i.test(ctx.agentText), 'agent never mentioned a PIN');

    const pinIdx = ctx.toolCalls.findIndex((t) => t.name === 'identify_employee' && t.args?.pin && !t.result?.error && t.result?.scope === 'full');
    ctx.check('PIN verified via identify_employee', pinIdx >= 0, JSON.stringify(ctx.toolCalls.filter((t) => t.name === 'identify_employee').map((t) => t.result)).slice(0, 200));

    // No internal reads before the PIN was verified.
    const schedIdxs = ctx.toolCalls.map((t, i) => (t.name === 'get_my_schedule' ? i : -1)).filter((i) => i >= 0);
    ctx.check(
      'no schedule read before PIN verification',
      pinIdx >= 0 && schedIdxs.every((i) => i > pinIdx),
      `PIN at ${pinIdx}, schedule calls at ${schedIdxs.join(',')}`
    );
    ...
    ctx.check(
      'no completion before the PIN was verified',
